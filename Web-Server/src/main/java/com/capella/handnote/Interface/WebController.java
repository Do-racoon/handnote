...
public class WebController {
    ...
    public String fileUpload(MultipartFile mFile, Integer index, Integer highlight){
        Image image = null;         // 이미지 파일
        List<TextInfo> textInfo = null;     // Request 텍스트 정보
        Integer maxline = 1;                // 최대 라인 수
        String text = Integer.toString(index) + "@";                   // 출력할 텍스트(HTML)

        try{
            System.out.println(index);
            image = new Image(mFile, index, highlight);
